...
const ProfessionalInfo = () => {
  return (
    <div className="widget-wrapper mb-0">
      <h6 className="title fz17 mb35 paragraph-theme">Professional Information</h6>
      {professionalInfoData.map((info, index) => (
        ...
        >
          <div className="flex-shrink-0">
            <h6 className="fz14 mb-0 text-light">{info.label}</h6>
          </div>
          <div className="news-content flex-shrink-1 ms-3 text-end">
            <p className="text mb-0 fz14 text-light">{info.content}</p>
          </div>
        </div>
      ))}
    </div>
